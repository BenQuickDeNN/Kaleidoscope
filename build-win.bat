...
REM Ԥ�����׶�
call %CC% -O3 -E %src_main% -o %build_main_i% -I %dir_include% && (goto prep_succeed) || goto prep_failed

:prep_succeed
REM ����׶�
call %CC% -S %build_main_i% -o %build_main_s% -g && (goto compile_succeed) || goto compile_failed

:compile_succeed
REM ���׶�
call %CC% -c %build_main_s% -o %build_main_o% && (goto asm_succeed) || goto asm_failed

:asm_succeed
REM ���ӽ׶�
call %CC% %build_main_o% `llvm-config --cxxflags --ldflags --system-libs --libs core`
